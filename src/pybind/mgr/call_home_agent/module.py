...
def performance(mgr: Any) -> dict:
    ...
    performance_metrics = {}
    perfo_interval_minutes = int(os.environ.get('CHA_INTERVAL_PERFORMANCE_REPORT_SECONDS',
                              mgr.get_module_option('interval_performance_report_seconds'))/60)

    try:
        queries = {
            "ceph_osd_op_r_avg"        : {"query": f"sum(avg_over_time(ceph_osd_op_r[{perfo_interval_minutes}m]))/count(ceph_osd_metadata)",
                                          "help" : f"Average of read operations per second and per OSD in the cluster in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_r_min"        : {"query": f"min(min_over_time(ceph_osd_op_r[{perfo_interval_minutes}m]))",
                                          "help" : f"Minimum read operations per second in the cluster in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_r_max"        : {"query": f"max(max_over_time(ceph_osd_op_r[{perfo_interval_minutes}m]))",
                                           "help": f"Maximum of write operations per second in the cluster in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_r_out_bytes_avg" : {"query": f"sum(avg_over_time(ceph_osd_op_r_out_bytes[{perfo_interval_minutes}m]))/count(ceph_osd_metadata)",
                                          "help" : f"Average of cluster output bytes(reads) and per OSD in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_r_out_bytes_min" : {"query": f"min(min_over_time(ceph_osd_op_r_out_bytes[{perfo_interval_minutes}m]))",
                                          "help" : f"Minimum of cluster output bytes(reads) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_r_out_bytes_max" : {"query": f"max(max_over_time(ceph_osd_op_r_out_bytes[{perfo_interval_minutes}m]))",
                                          "help" : f"Maximum of cluster output bytes(reads) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_avg"        : {"query": f"sum(avg_over_time(ceph_osd_op_w[{perfo_interval_minutes}m]))/count(ceph_osd_metadata)",
                                          "help" : f"Average of cluster input operations per second(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_min"        : {"query": f"min(min_over_time(ceph_osd_op_w[{perfo_interval_minutes}m]))",
                                          "help" : f"Mimimum of cluster input operations per second(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_max"        : {"query": f"max(max_over_time(ceph_osd_op_w[{perfo_interval_minutes}m]))",
                                          "help" : f"Maximum of cluster input operations per second(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_in_bytes_avg"       : {"query": f"sum(avg_over_time(ceph_osd_op_w_in_bytes[{perfo_interval_minutes}m]))/count(ceph_osd_metadata)",
                                                  "help" : f"Average of cluster input bytes(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_in_bytes_min"       : {"query": f"min(min_over_time(ceph_osd_op_w_in_bytes[{perfo_interval_minutes}m]))",
                                                  "help" : f"Minimum of cluster input bytes(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_w_in_bytes_max"       : {"query": f"max(max_over_time(ceph_osd_op_w_in_bytes[{perfo_interval_minutes}m]))",
                                                  "help" : f"Maximum of cluster input bytes(writes) in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_read_latency_avg_ms"  : {"query": f"avg(rate(ceph_osd_op_r_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_r_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Average of cluster output latency(reads) in milliseconds in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_read_latency_max_ms"  : {"query": f"max(rate(ceph_osd_op_r_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_r_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Maximum of cluster output latency(reads) in milliseconds in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_read_latency_min_ms"  : {"query": f"min(rate(ceph_osd_op_r_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_r_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Minimum of cluster output latency(reads) in milliseconds  in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_write_latency_avg_ms" : {"query": f"avg(rate(ceph_osd_op_w_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_w_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Average of cluster input latency(writes) in milliseconds in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_write_latency_max_ms" : {"query": f"max(rate(ceph_osd_op_w_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_w_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Maximum of cluster input latency(writes) in milliseconds  in the last {perfo_interval_minutes} minutes"},
            "ceph_osd_op_write_latency_min_ms" : {"query": f"min(rate(ceph_osd_op_w_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_osd_op_w_latency_count[{perfo_interval_minutes}m]) * 1000)",
                                                  "help" : f"Maximum of cluster input latency(writes) in milliseconds in the last {perfo_interval_minutes} minutes"},
            ...
            "ceph_pool_write_iops"  : {"query": f"rate(ceph_pool_wr[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : "Per-second average rate of increase of write operations per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pool_read_iops"   : {"query": f"rate(ceph_pool_rd[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of increase of read operations per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pool_write_bytes" : {"query": f"rate(ceph_pool_wr_bytes[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of increase of written bytes per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pool_read_bytes"  : {"query": f"rate(ceph_pool_rd_bytes[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of increase of read bytes per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pg_activating"    : {"query": f"rate(ceph_pg_activating[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of Placement Groups activated per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pg_backfilling"   : {"query": f"rate(ceph_pg_backfilling[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of Placement Groups backfilled per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pg_creating"      : {"query": f"rate(ceph_pg_creating[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of Placement Groups created per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pg_recovering"    : {"query": f"rate(ceph_pg_recovering[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help" : f"Per-second average rate of Placement Groups recovered per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_pg_deep"          : {"query": f"rate(ceph_pg_deep[{perfo_interval_minutes}m]) * on(pool_id) group_left(instance, name) ceph_pool_metadata",
                                       "help":  f"Per-second average rate of Placement Groups deep scrubbed per Ceph pool during the last {perfo_interval_minutes} minutes"},
            "ceph_rgw_avg_get_latency_ms" : {"query": f'(rate(ceph_rgw_get_initial_lat_sum[{perfo_interval_minutes}m]) or vector(0)) * 1000 / rate(ceph_rgw_get_initial_lat_count[{perfo_interval_minutes}m]) * on (instance_id) group_left (ceph_daemon) ceph_rgw_metadata',
                                             "help" : f"Average latency in milliseconds for GET operations per Ceph RGW daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_rgw_avg_put_latency_ms" : {"query": f"(rate(ceph_rgw_put_initial_lat_sum[{perfo_interval_minutes}m]) or vector(0)) * 1000 / rate(ceph_rgw_put_initial_lat_count[{perfo_interval_minutes}m]) * on (instance_id) group_left (ceph_daemon) ceph_rgw_metadata",
                                             "help" : f"Average latency in milliseconds for PUT operations per Ceph RGW daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_rgw_requests_per_second": {"query": f'sum by (rgw_host) (label_replace(rate(ceph_rgw_req[{perfo_interval_minutes}m]) * on (instance_id) group_left (ceph_daemon) ceph_rgw_metadata, "rgw_host", "$1", "ceph_daemon", "rgw.(.*)"))',
                                             "help" : f"Request operations per second per Ceph RGW daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_rgw_get_size_bytes" :     {"query": f'label_replace(sum by (instance_id) (rate(ceph_rgw_get_b[{perfo_interval_minutes}m])) * on (instance_id) group_left (ceph_daemon) ceph_rgw_metadata, "rgw_host", "$1", "ceph_daemon", "rgw.(.*)")',
                                             "help" : f"Per-second average rate of GET operations size per Ceph RGW daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_rgw_put_size_bytes" :     {"query": f'label_replace(sum by (instance_id) (rate(ceph_rgw_put_b[{perfo_interval_minutes}m])) * on (instance_id) group_left (ceph_daemon) ceph_rgw_metadata, "rgw_host", "$1", "ceph_daemon", "rgw.(.*)")',
                                             "help" : f"Per-second average rate of PUT operations size per Ceph RGW daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_read_requests_per_second"   : {"query": f'rate(ceph_objecter_op_r{{ceph_daemon=~"mds.*"}}[{perfo_interval_minutes}m])',
                                                     "help" : f"Per-second average rate of read requests per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_write_requests_per_second"  : {"query": f'rate(ceph_objecter_op_w{{ceph_daemon=~"mds.*"}}[{perfo_interval_minutes}m])',
                                                     "help" : f"Per-second average rate of write requests per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_client_requests_per_second" : {"query": f'rate(ceph_mds_server_handle_client_request[{perfo_interval_minutes}m])',
                                                     "help" : f"Per-second average rate of client requests per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_reply_latency_avg_ms" : {"query": f'avg(rate(ceph_mds_reply_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_mds_reply_latency_count[{perfo_interval_minutes}m]) * 1000)',
                                               "help" : f"Average of the per-second average rate of reply latency(seconds) per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_reply_latency_max_ms" : {"query": f'max(rate(ceph_mds_reply_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_mds_reply_latency_count[{perfo_interval_minutes}m]) * 1000)',
                                               "help" : f"Maximum of the per-second average rate of reply latency(seconds) per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "ceph_mds_reply_latency_min_ms" : {"query": f'min(rate(ceph_mds_reply_latency_sum[{perfo_interval_minutes}m]) or vector(0) / on (ceph_daemon) rate(ceph_mds_reply_latency_count[{perfo_interval_minutes}m]) * 1000)',
                                               "help" : f"Minimum of the per-second average rate of reply latency(seconds) per Ceph MDS daemon during the last {perfo_interval_minutes} minutes"},
            "hw_cpu_busy"                          : {"query": f"1- rate(node_cpu_seconds_total{{mode='idle'}}[{perfo_interval_minutes}m])",
                                                      "help" : f"Percentaje of CPU utilization per core during the last {perfo_interval_minutes} minutes"},
            ...
            "hw_node_physical_disk_read_ops_rate"  : {"query": f"rate(node_disk_reads_completed_total[{perfo_interval_minutes}m])",
                                                      "help" : f"Per-second average rate of read operations per physical storage device in the host during the last {perfo_interval_minutes} minutes"},
            "hw_node_physical_disk_write_ops_rate" : {"query": f"rate(node_disk_writes_completed_total[{perfo_interval_minutes}m])",
                                                      "help" : f"Per-second average rate of write operations per physical storage device in the host during the last {perfo_interval_minutes} minutes"},
            "hw_disk_utilization_rate"             : {"query": f"rate(node_disk_io_time_seconds_total[{perfo_interval_minutes}m])",
                                                      "help" : f"Per-second average rate of input/output operations time(seconds) per physical storage device in the host during the last {perfo_interval_minutes} minutes"},
            "hw_network_bandwidth_receive_load_bytes" : {"query": f"rate(node_network_receive_bytes_total[{perfo_interval_minutes}m])",
                                                         "help" : f"Per-second average rate of received bytes per network card in the host during the last {perfo_interval_minutes} minutes"},
            "hw_network_bandwidth_transmit_load_bytes": {"query": f"rate(node_network_transmit_bytes_total[{perfo_interval_minutes}m])",
                                                         "help" : f"Per-second average rate of transmitted bytes per network card in the host during the last {perfo_interval_minutes} minutes"},
            ...
            "ceph_nvmeof_gateway_reactor_cpu_seconds_total"    : {"query": f'max by(group) (avg by(instance) (rate(ceph_nvmeof_reactor_seconds_total{{mode="busy"}}[{perfo_interval_minutes}m])) * on(instance) group_right ceph_nvmeof_gateway_info)',
            ...
            "ceph_nvmeof_gateway_iops_total "                  : {"query": f"sum by(instance) (rate(ceph_nvmeof_bdev_reads_completed_total[{perfo_interval_minutes}m]) + rate(ceph_nvmeof_bdev_writes_completed_total[{perfo_interval_minutes}m])) * on(instance) group_left(group) ceph_nvmeof_gateway_info",
                                                                  "help" : "IOPS per Ceph NVMe-oF gateway"},
            "ceph_nvmeof_subsystem_iops_total"                 : {"query": f"sum by(group,nqn) (((rate(ceph_nvmeof_bdev_reads_completed_total[{perfo_interval_minutes}m]) + rate(ceph_nvmeof_bdev_writes_completed_total[{perfo_interval_minutes}m])) * on(instance,bdev_name) group_right ceph_nvmeof_subsystem_namespace_metadata) * on(instance) group_left(group) ceph_nvmeof_gateway_info)",
                                                                  "help" : "IOPS per Ceph NVMe-oF subsystem"},
            "ceph_nvmeof_gateway_throughput_bytes_total"       : {"query": f"sum by(instance) (rate(ceph_nvmeof_bdev_read_bytes_total[{perfo_interval_minutes}m]) + rate(ceph_nvmeof_bdev_written_bytes_total[{perfo_interval_minutes}m])) * on(instance) group_left(group) ceph_nvmeof_gateway_info",
                                                                  "help" : "Throughput per Ceph NVMe-oF gateway"},
            "ceph_nvmeof_subsystem_throughput_bytes_total"     : {"query": f"sum by(group,nqn) (((rate(ceph_nvmeof_bdev_read_bytes_total[{perfo_interval_minutes}m]) + rate(ceph_nvmeof_bdev_written_bytes_total[{perfo_interval_minutes}m])) * on(instance,bdev_name) group_right ceph_nvmeof_subsystem_namespace_metadata) * on(instance) group_left(group) ceph_nvmeof_gateway_info)",
                                                                  "help" : "Throughput per Ceph NVMe-oF subsystem"},
            "ceph_nvmeof_gateway_read_avg_latency_seconds"     : {"query": f"avg by(group,instance) (((rate(ceph_nvmeof_bdev_read_seconds_total[{perfo_interval_minutes}m]) / rate(ceph_nvmeof_bdev_reads_completed_total[{perfo_interval_minutes}m])) > 0) * on(instance) group_left(group) ceph_nvmeof_gateway_info)",
                                                                  "help" : "Read latency average in seconds per Ceph NVMe-oF gateway"},
            "ceph_nvmeof_gateway_write_avg_latency_seconds "   : {"query": f"avg by(group,instance) (((rate(ceph_nvmeof_bdev_write_seconds_total[{perfo_interval_minutes}m]) / rate(ceph_nvmeof_bdev_writes_completed_total[{perfo_interval_minutes}m])) > 0) * on(instance) group_left(group) ceph_nvmeof_gateway_info)",
                                                                  "help":  "Write average in seconds per Ceph NVMe-oF gateway"},
            "ceph_nvmeof_gateway_read_p95_latency_seconds"     : {"query": f"quantile by(group,instance) (.95,((rate(ceph_nvmeof_bdev_read_seconds_total[{perfo_interval_minutes}m]) / (rate(ceph_nvmeof_bdev_reads_completed_total[{perfo_interval_minutes}m]) >0)) * on(instance) group_left(group) ceph_nvmeof_gateway_info))",
                                                                  "help":  "Read latency for 95{%} of the Ceph NVMe-oF gateways"},
            "ceph_nvmeof_gateway_write_p95_latency_seconds"    : {"query": f"quantile by(group,instance) (.95,((rate(ceph_nvmeof_bdev_write_seconds_total[{perfo_interval_minutes}m]) / (rate(ceph_nvmeof_bdev_writes_completed_total[{perfo_interval_minutes}m]) >0)) * on(instance) group_left(group) ceph_nvmeof_gateway_info))",
